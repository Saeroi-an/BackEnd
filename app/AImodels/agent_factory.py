...
# 전역 변수 (변수명 변경 금지!)
huggingfacehub = None  # 전역 LLM 인스턴스
GLOBAL_TOOLS = []
initial_agent = None   # 초기화 완료 플래그

def initialize_global_agent():
    """전역 LLM과 Tool을 초기화 (앱 시작 시 단 한 번만 호출)"""
    global huggingfacehub, GLOBAL_TOOLS, initial_agent
    
    try:
        print("🚀 Initializing Global LLM and Tools...")
        ...
        print(f"✅ LLM initialized: {REPO_ID}")
        print(f"✅ Tools loaded: {[tool.name for tool in GLOBAL_TOOLS]}")
...
def cleanup_old_sessions(max_sessions: int = 1000):
    ...
    if len(SESSION_MEMORY_CACHE) > max_sessions:
        ...
        print(f"🧹 Cleaned up {len(keys_to_delete)} old sessions")